 ...
 * Program types.
 *
 * See {@link \WlSdk\RsProgramSid} for a list of promotions.
 *
 * Last used ID: 8.
 *
 * Values:
 ...
 *
 *   Differs from {@link \WlSdk\RsProgramTypeSid} in that this type of promotion is limited by time.
 * - 8 (`GUEST`): Guest passes.
 * - 1 (`LIMIT`): Class pass.
 *
 *   Differs from {@link \WlSdk\RsProgramTypeSid} in that this type of promotion allows access to only a limited number
 *   of classes.
 * - 3 (`MEMBERSHIP`): This promotion is a membership.
 *
 *   Payment schedule can be set up for memberships.
 * - 6 (`OTHER`): Type for programs that are not presented in {@link \WlSdk\RsProgramSid}.
 * - 4 (`PACKAGE`): Packages and Daily deals.
 * - 2 (`PASS`): Unlimited pass. Day/week/month pass.
 *
 *   Allows access to unlimited number of classes.
 *
 *   Differs from {@link \WlSdk\RsProgramTypeSid} in that this type of promotion can only be paid once.
...
class RsProgramTypeSid
{
    ...
    /** Type for programs that are not presented in {@link \WlSdk\RsProgramSid}. */
