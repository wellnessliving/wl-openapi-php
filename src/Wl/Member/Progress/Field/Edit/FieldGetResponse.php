...
class FieldGetResponse
{
    ...
    {
        $this->a_measurement = isset($data['a_measurement']) ? array_map(static fn($v) => \WlSdk\Wl\Member\Progress\Field\MeasurementSid::tryFrom((int)$v), (array)$data['a_measurement']) : null;
        $this->a_type = isset($data['a_type']) ? array_map(static fn($v) => \WlSdk\Wl\Member\Progress\Field\TypeSid::tryFrom((int)$v), (array)$data['a_type']) : null;
